...
// trusted unix socket to discover the loopback listener URL and token path,
// reads the group-readable token, and opens the browser at the token-handoff
// URL so the loaded UI can authenticate its API calls.
...
func (cmd *uiCommand) run(_ *cobra.Command, _ []string) error {
	...
	fmt.Printf("Opening the UI at %s\n", ui.URL)
	fmt.Printf("If your browser does not open automatically, visit:\n\n  %s\n", loginURL)
	openBrowser(loginURL)
	return nil
}

// reportDisabled explains how to enable the UI listener rather than failing
// silently (task 7.2).
...
// uiURL is like http://127.0.0.1:port/ui/; the login endpoint sits at /ui/login.
func buildLoginURL(uiURL, token string) (string, error) {
	base := strings.TrimSuffix(uiURL, "/")
