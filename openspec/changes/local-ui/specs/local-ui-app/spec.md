...
and a chat module) using the browser `fetch` API. The client SHALL interpret the daemon's
response envelope: reading `metadata` from `sync` responses, the operation reference from
`async` responses, and surfacing `error` responses as typed errors.
