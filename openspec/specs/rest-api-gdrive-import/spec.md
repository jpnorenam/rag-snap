# rest-api-gdrive-import Specification

## Purpose

Expose Google Drive import over the daemon API so the browser UI reaches parity with the CLI's
`k import --url`. This covers the Drive OAuth lifecycle (start/status/disconnect) mediated over
REST, resolving a Drive folder or file URL into the discovered `.tar.gz` archives, and importing
selected archives as tracked operations. It touches OpenSearch (the import target) and Google's
OAuth/Drive HTTP APIs; it does not touch the inference server or Tika.
