# prompt-cli Specification

## Purpose

Expose the daemon's prompt store through the CLI so users can manage named prompt variants —
create, edit, activate, inspect history, restore, and delete them — from scriptable
subcommands under `prompt`, and select a variant for a single chat session or batch run. Variant
management requires the daemon and never falls back to the client-local prompts file; the
daemonless `prompt init` flow keeps its existing single-override behavior.
