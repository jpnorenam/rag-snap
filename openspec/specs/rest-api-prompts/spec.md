# rest-api-prompts Specification

## Purpose

Expose the three generation prompt templates — `chat_system_prompt`, `answer_system_prompt`,
and `source_rules` — over the API so clients can read their effective values, customize them,
and reset them to the built-in defaults. The two generation slots additionally carry named
variants with append-only version histories and a per-slot active pointer selecting which
variant (or the built-in default) is effective. The daemon owns the persisted store; a slot
with no active variant always resolves to the built-in default of the running release. The
chat and batch-answer capabilities read their generation prompts from this store.
...
— a slot with no active variant always resolves to the built-in default of the running release.
A persisted record that cannot be parsed SHALL NOT fail prompt resolution; the daemon SHALL
