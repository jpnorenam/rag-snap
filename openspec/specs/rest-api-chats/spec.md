# rest-api-chats Specification

## Purpose

Expose saved chats as a REST collection on the daemon so clients can list, search, fetch,
and delete persisted conversations. The store lives under `$SNAP_COMMON` alongside the
prompt store, keyed per chat so it survives snap refreshes and tolerates corrupt files.
