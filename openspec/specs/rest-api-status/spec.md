# rest-api-status Specification

## Purpose

Expose the health of the three external services the CLI orchestrates — OpenSearch (the
knowledge store), the inference server (the chat backend), and Tika (text extraction) — plus the
daemon's own listener state, over the API. This is the API-side equivalent of the
`rag-cli.rag status` command: live probes at request time, per-service degradation, and the
OpenSearch ML model detail needed to tell a configured model ID from a deployed one.
