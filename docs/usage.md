...
The `extract-rfp` command parses an RFP or RFI document (PDF, DOCX, XLSX, or CSV), guides you
through format-specific extraction options, and produces a YAML manifest ready to feed into
`answer batch`. It is the recommended way to turn a vendor questionnaire into an answerable batch
job without manually copying questions.

### Sub-commands at a glance

| Command | Description |
|---|---|
| `extract-rfp <document>` | Extract questions and write a batch manifest |
| `extract-rfp <document> --preview` | Dry-run: show extracted questions without writing any file |
...
```
rag-cli.rag extract-rfp <document-path> [--output <path>] [--preview]
```
...
```yaml
# Generated by rag-cli extract-rfp on 2026-04-29 10:00:00 UTC
version: "1.0"
knowledge_bases:
  - project-docs
questions:
...
$ rag-cli.rag extract-rfp ~/rfp/vendor-questionnaire.xlsx
...
$ rag-cli.rag extract-rfp ~/rfp/orange-rfp.pdf
...
rag-cli.rag extract-rfp ~/rfp/ericsson.pdf --preview
```
